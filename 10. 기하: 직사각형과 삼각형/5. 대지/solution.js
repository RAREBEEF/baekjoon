...
// 이미지 여부: true
//
// 원본 이미지 0: https://onlinejudgeimages.s3.amazonaws.com/problem/9063/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202017-01-03%20%EC%98%A4%ED%9B%84%204.58.37.png
...
const solution = (marbles) => {
  const marblesCount = marbles.shift();
  const land = { x1: null, y1: null, x2: null, y2: null };

  for (let i = 0; i < marblesCount; i++) {
    const [x, y] = marbles[i];
    land.x1 = land.x1 ? Math.min(land.x1, x) : x;
    land.x2 = land.x2 ? Math.max(land.x2, x) : x;
    land.y1 = land.y1 ? Math.min(land.y1, y) : y;
    land.y2 = land.y2 ? Math.max(land.y2, y) : y;
  }

  return (land.x2 - land.x1) * (land.y2 - land.y1);
};
